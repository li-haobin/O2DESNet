...
namespace O2DESNet.Benchmarks
{
    public abstract class DTLZx : Benchmark
    {
        ...
        {
            NumObjectives = noiseLevels.Length;
            // feasibility check
            bool feasible = Dimension > 0 && NumObjectives > 0;
    ...
    public class DTLZ1 : DTLZx
    {
        ...
        internal override double[] CalObjectives()
        {
            ...
            int k = Dimension - NumObjectives + 1;
    ...
    public class DTLZ5 : DTLZx
    {
        ...
        internal override double[] CalObjectives()
        {
            ...
            int k = Dimension - NumObjectives + 1;
            double[] f = new double[NumObjectives];
            double[] theta = new double[NumObjectives - 1];
            double g = 0.0;
            for (int i = Dimension - k; i < Dimension; i++)
                g += (x[i] - 0.5) * (x[i] - 0.5);
            double t = Math.PI / (4.0 * (1.0 + g));
            theta[0] = x[0] * Math.PI / 2.0;
            for (int i = 1; i < NumObjectives - 1; i++)
