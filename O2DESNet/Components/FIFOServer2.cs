...
namespace O2DESNet
{
    public class FIFOServer2<TLoad> : Component<FIFOServer2<TLoad>.Statics>
        where TLoad : Load
    {
        #region Statics
        ...
        {
            public FIFOServer2<TLoad> FIFOServer { get; private set; }
            public TLoad Load { get; private set; }
            internal StartEvent(FIFOServer2<TLoad> fifoServer, TLoad load)
            {
        ...
        private class FinishEvent : Event
        {
            public FIFOServer2<TLoad> FIFOServer { get; private set; }
            public TLoad Load { get; private set; }
            internal FinishEvent(FIFOServer2<TLoad> fifoServer, TLoad load)
            {
                FIFOServer = fifoServer;
                Load = load;
            }
        ...
        {
            public FIFOServer2<TLoad> FIFOServer { get; private set; }
            internal DepartEvent(FIFOServer2<TLoad> fifoServer)
            {
                FIFOServer = fifoServer;
            }
            public override void Invoke()
        ...
        public FIFOServer2(Statics config, int seed, string tag = null) : base(config, seed, tag)
        {
            Name = "FIFOServer";
