...
namespace O2DESNet.Demos.SynchronizedProcess
{
    public class SynchronizedProcess : Component<SynchronizedProcess.Statics>
    {
        ...
        public class Statics : Scenario
        {
            ...
            public Queue<Load>.Statics QueueA { get; set; }
            public Queue<Load>.Statics QueueB { get; set; }
        ...
        public Queue<Load> QueueA { get; private set; }
        public Queue<Load> QueueB { get; private set; }
        ...
        public SynchronizedProcess(Statics config, int seed, string tag = null) : base(config, seed, tag)
        {
            ...
            QueueA = new Queue<Load>(config.QueueA, "Queue A");
            QueueA.OnDequeue.Add(load => ServerA1.Start(load));

            QueueB = new Queue<Load>(config.QueueB, "Queue B");
            ...
            ServerA1.OnStateChange.Add(s => QueueA.UpdToDequeue(s.Vacancy > 0));
            ServerA1.OnStateChange.Add(s => Sync.UpdState(1, s.Served.Count > 0)); // condition 1

            ServerB1 = new Server<Load>(config.ServerB1, DefaultRS.Next(), "Server B1");
            ServerB1.OnDepart.Add(load => ServerB2.Start(load));
            ServerB1.OnStateChange.Add(s => QueueB.UpdToDequeue(s.Vacancy > 0));
            ServerB1.OnStateChange.Add(s => Sync.UpdState(2, s.Served.Count > 0)); // condition 2

            Sync = new Synchronizer(new Synchronizer.Statics(4));
            Sync.OnStateChange.Add(s => ServerA1.UpdToDepart(s.AllTrue)); // action 1
            Sync.OnStateChange.Add(s => ServerB1.UpdToDepart(s.AllTrue)); // action 2
            ...
            ServerA2.OnStateChange.Add(s => Sync.UpdState(3, s.Vacancy > 0)); // condition 3
            ...
            ServerB2.OnStateChange.Add(s => Sync.UpdState(4, s.Vacancy > 0)); // condition 4
