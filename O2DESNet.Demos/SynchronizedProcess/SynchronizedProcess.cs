...
{
    public class SynchronizedProcess : State<SynchronizedProcess.Statics>
    {
        #region Statics
        ...
        public SynchronizedProcess(Statics config, int seed, string tag = null) : base(config, seed, tag)
        {
            ...
            ServerA1.OnStateChg.Add(() => QueueA.UpdToDequeue(ServerA1.Vacancy > 0));
            ServerA1.OnStateChg.Add(() => Sync.UpdState(1, ServerA1.Served.Count > 0)); // condition 1

            ServerB1 = new Server<Load>(config.ServerB1, DefaultRS.Next(), "Server B1");
            ServerB1.OnDepart.Add(load => ServerB2.Start(load));
            ServerB1.OnStateChg.Add(() => QueueB.UpdToDequeue(ServerB1.Vacancy > 0));
            ServerB1.OnStateChg.Add(() => Sync.UpdState(2, ServerB1.Served.Count > 0)); // condition 2

            Sync = new Synchronizer(new Synchronizer.Statics(4));
            Sync.OnStateChg.Add(() => ServerA1.UpdToDepart(Sync.AllTrue)); // action 1
            Sync.OnStateChg.Add(() => ServerB1.UpdToDepart(Sync.AllTrue)); // action 2
            ...
            ServerA2.OnStateChg.Add(() => Sync.UpdState(3, ServerA2.Vacancy > 0)); // condition 3
            ...
            ServerB2.OnStateChg.Add(() => Sync.UpdState(4, ServerB2.Vacancy > 0)); // condition 4
